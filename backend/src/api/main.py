...
# CRM service for marketing campaigns
from src.core.services.crm_service import CRMService
...
class CampaignRequest(BaseModel):
    """Request model for sending or previewing campaigns."""
    days: Optional[int] = Field(
        None,
        description="Número de días a considerar (adelante o hacia atrás, según campaña)",
    )

class SMTPTestRequest(BaseModel):
    """Request model for SMTP test email."""
    to: str = Field(..., description="Correo destino para la prueba SMTP del CRM")
...
# Integración de campañas de CRM (cumpleaños e inactivos) en el mismo backend

@app.get("/crm/campaigns/birthdays")
async def crm_get_birthdays(days: int = 7):
    """
    Devuelve campañas de cumpleaños para los próximos `days` días
    (sin enviar correos, solo vista previa).
    """
    try:
        campaigns = crm_service.prepare_birthday_campaigns(days_ahead=days)
        return {"campaigns": campaigns, "count": len(campaigns)}
    except Exception as e:
        logger.error(f"Error getting birthday campaigns: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/crm/campaigns/birthdays/send")
async def crm_send_birthdays(request: CampaignRequest):
    """
    Envía por correo las campañas de cumpleaños.
    """
    try:
        days = request.days or 7
        result = crm_service.send_birthday_campaigns(days_ahead=days)
        return result
    except Exception as e:
        logger.error(f"Error sending birthday campaigns: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/crm/campaigns/inactive")
async def crm_get_inactive(days_since: int = 30):
    """
    Devuelve campañas para clientes inactivos (no han venido en > days_since días).
    """
    try:
        campaigns = crm_service.prepare_inactive_campaigns(days_since=days_since)
        return {"campaigns": campaigns, "count": len(campaigns)}
    except Exception as e:
        logger.error(f"Error getting inactive campaigns: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/crm/campaigns/inactive/send")
async def crm_send_inactive(request: CampaignRequest):
    """
    Envía por correo campañas para clientes inactivos.
    """
    try:
        days = request.days or 30
        result = crm_service.send_inactive_campaigns(days_since=days)
        return result
    except Exception as e:
        logger.error(f"Error sending inactive campaigns: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/crm/db_check")
async def crm_db_check():
    """
    Verifica la conexión a la base de datos usada por el CRM.
    """
    result = crm_service.db_check()
    status_code = 200 if result.get("ok") else 500
    return JSONResponse(result, status_code=status_code)


@app.get("/crm/smtp_check")
async def crm_smtp_check():
    """
    Verifica que la configuración SMTP sea válida (login).
    """
    result = crm_service.smtp_check()
    status_code = 200 if result.get("ok") else 500
    return JSONResponse(result, status_code=status_code)


@app.post("/crm/send_test_email")
async def crm_send_test_email(request: SMTPTestRequest):
    """
    Envía un correo de prueba para validar la entrega SMTP.
    """
    result = crm_service.send_test_email(request.to)
    status_code = 200 if result.get("ok") else 500
    return JSONResponse(result, status_code=status_code)
...
async def crm_get_customers(limit: int = 200):
    """
    Devuelve overview de clientes para el dashboard de CRM.
    """
    try:
        customers = crm_service.get_customers_overview(limit=limit)
...
# --- WhatsApp Router (Twilio integration) ---
from src.api.routes.whatsapp_router import router as whatsapp_router
