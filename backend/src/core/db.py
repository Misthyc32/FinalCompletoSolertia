...
def save_message(session, customer_id: str, role: str, content: str):
    # Try to upsert user first (handle both numeric and WhatsApp-style IDs)
    try:
        # If it's a WhatsApp ID like "whatsapp:+521234567890", extract digits only
        if isinstance(customer_id, str) and "whatsapp" in customer_id:
            digits_only = ''.join(filter(str.isdigit, customer_id))
            if digits_only:
                customer_id_int = int(digits_only)
                upsert_user(session, customer_id=customer_id_int)
                print(f"✅ Created/updated user for WhatsApp ID → {customer_id_int}")
            else:
                print(f"⚠️ No digits found in {customer_id}, skipping user creation.")
        # If it's already numeric, just use it
        elif str(customer_id).isdigit():
            upsert_user(session, customer_id=int(customer_id))
            print(f"✅ Created/updated numeric user: {customer_id}")
        else:
            print(f"⚠️ Unrecognized ID format: {customer_id}, skipping user creation.")
    except Exception as e:
        # If user creation fails, log but continue saving message
        print(f"⚠️ Could not create user for customer_id={customer_id}: {e}")
